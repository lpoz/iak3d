...
source(paste0(iakDir , '/getOodnadattaData.R')) 
...
# Ood: 1 rule, indicating linear model fits best
...
###########################################################################
## LPOZ: Depths used at Ood are 0-15,15-30,30-60,60-90cm
...
##### LPOZ: Validation is hard coded to predict to 2m, need to fix this. 
...
if(valNow){
  ...
  dIPred <- cbind(seq(0 , 1.98 , 0.02) , seq(0.02 , 2 , 0.02))  # keep same until backend fixed?
  ...
  tmp <- calcValStats(zVal = zVal , dIVal = dIVal , zkVal = zkVal , vkVal = vkVal , layerMidPts = c(0.075 , 0.225 , 0.45 , 0.75) , printValStats = TRUE)
